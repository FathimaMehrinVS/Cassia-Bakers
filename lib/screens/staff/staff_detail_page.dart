...
class _StaffDetailPageState extends State<StaffDetailPage> {
  // ── Calculation Helpers ────────────────────────────────────────────────────
  List<double> _calculateRunningBalances(List<StaffTransaction> transactions) {
  ...
  void _navigateToEntry({required bool isPayment, required double currentBalance}) {
      ...
      MaterialPageRoute(
        builder: (context) => TransactionEntryPage(
          ...
          onConfirm: (amount, notes, date, imageBytes, imageName) async {
            ...
            try {
              await StaffService().addTransaction(widget.staff.id, tx);
              widget.onChanged();
              _showSuccessSnackbar('Transaction recorded successfully!');
            } catch (e) {
              _showErrorSnackbar('Failed to record transaction: $e');
            }
          },
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot>(
      stream: FirebaseFirestore.instance.collection('staff').doc(widget.staff.id).snapshots(),
      builder: (context, staffSnapshot) {
        if (!staffSnapshot.hasData) {
          return const Scaffold(body: Center(child: CircularProgressIndicator()));
        }
        final s = StaffData.fromFirestore(staffSnapshot.data!);
        final initials = s.name.isNotEmpty ? s.name[0].toUpperCase() : 'S';
        ...
        return Scaffold(
          ...
          body: StreamBuilder<List<StaffTransaction>>(
            stream: StaffService().getTransactions(s.id),
                            ...
                            : ListView.builder(
                                ...
                                itemBuilder: (context, index) {
                                  ...
                                  if (index == 0) {
                                    ...
                                    final prevDatePart = transactionsList[index - 1].date.split(' at ')[0];
                              ...
                              children: [
                                ...
                                ElevatedButton.icon(
                                  ...
                                  onPressed: () => _showSmsReminder(
                                    ...
                                    dueAmount: widget.staff.balance,
                                    label: 'staff member',
                                  ),
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    Text(
                                      s.balance >= 0
                                          ? '₹ ${s.balance.toStringAsFixed(0)} Salary Due'
                                          : '₹ ${(-s.balance).toStringAsFixed(0)} Advance',
                                      style: TextStyle(
                                        fontSize: 15,
                                        fontWeight: FontWeight.bold,
                                        color: s.balance >= 0 ? Colors.green[800] : Colors.blue[800],
                                      ),
                                    ),
                                    const SizedBox(width: 4),
                                    Icon(
                          ...
                          Container(
                            ...
                            child: Row(
                              children: [
                                ...
                                Expanded(
                                  child: InkWell(
                                    onTap: () => _navigateToEntry(isPayment: true, currentBalance: s.balance),
                                    borderRadius: BorderRadius.circular(8),
                                    child: Container(
                                      height: 48,
                                      alignment: Alignment.center,
                                ...
                                Expanded(
                                  child: InkWell(
                                    onTap: () => _navigateToEntry(isPayment: false, currentBalance: s.balance),
                                    borderRadius: BorderRadius.circular(8),
                                    child: Container(
                                      height: 48,
                                      alignment: Alignment.center,
  ...
  Future<void> _showSmsReminder(
    ...
    final defaultMsg =
        ...
        'You have an outstanding salary due of ₹${dueAmount.toStringAsFixed(0)}. '
        'Kindly contact us at your earliest convenience. Thank you!';
