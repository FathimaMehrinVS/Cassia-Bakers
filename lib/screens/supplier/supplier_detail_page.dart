...
class _SupplierDetailPageState extends State<SupplierDetailPage> {
  ...
  List<double> _calculateRunningBalances() {
    ...
    double current = 0.0;
    for (final tx in widget.supplier.transactions) {
      if (tx.isPayment) {
  ...
  void _saveLedgerTransaction() {
    ...
    setState(() {
      widget.supplier.transactions.add(SupplierTransaction(
  ...
  void _saveBill() {
    ...
    // Also add a transaction entry so the due is updated in the ledger
    final now = DateTime.now();
    final timeStr =
        '${now.hour.toString().padLeft(2, '0')}:${now.minute.toString().padLeft(2, '0')} ${now.hour >= 12 ? 'PM' : 'AM'}';
    final dateStr = '${now.day} ${_getMonthName(now.month)} ${now.year}';
    ...
    setState(() {
      widget.supplier.bills.add(newBill);
      widget.supplier.transactions.add(SupplierTransaction(
    ...
    _showSuccessSnackbar(
        'Bill $billNo saved! Due updated in ledger.');
  }
  ...
  Widget build(BuildContext context) {
    final initials = widget.supplier.name.isNotEmpty
        ? widget.supplier.name[0].toUpperCase()
        : 'S';
    final netDue = widget.supplier.netDue;
    ...
    return Scaffold(
      ...
      body: _activeTab == 0 ? _buildLedgerTab() : _buildAddBillTab(),
    );
  }
          ...
          child: widget.supplier.transactions.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    final tx = widget.supplier.transactions[index];
                    ...
                    if (index == 0) {
                      ...
                      final prevDatePart = widget.supplier.transactions[index - 1]
                          .date
                          .split(' at ')[0];
              ...
              child: Row(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Text(
                        widget.supplier.netDue > 0
                            ? '₹ ${widget.supplier.netDue.toStringAsFixed(0)} You Pay'
                            : widget.supplier.netDue < 0
                                ? '₹ ${(-widget.supplier.netDue).toStringAsFixed(0)} Advance'
                                : 'Settled',
                        style: TextStyle(
                          fontSize: 15,
                          fontWeight: FontWeight.bold,
                          color: widget.supplier.netDue > 0
                              ? Colors.red[700]
                              : Colors.green[800],
                        ),
                      ),
                        ...
                        Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (context) => TransactionEntryPage(
                              ...
                              currentDue: widget.supplier.netDue,
                              dueLabel: 'Due',
                              isPayment: true,
                              avatarInitial: widget.supplier.name.isNotEmpty
                                  ? widget.supplier.name[0].toUpperCase()
                                  : 'S',
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String>(
                          ...
                          items: [
                            DropdownMenuItem(
                              value: widget.supplier.id,
                              child: Text(widget.supplier.name, overflow: TextOverflow.ellipsis),
                            )
                          ],
                          onChanged: (_) {},
                        ),
